...
<head>
	...
	function doSelectAll(){
  	...
  	//新增
  	function doAdd(){
  		document.forms[0].action = "${basePath}manage/info_addUI.action";
  		document.forms[0].submit();
  	}
  	//编辑
  	function doEdit(id){
  		document.forms[0].action = "${basePath}manage/info_editUI.action?info.infoId=" + id;
  		document.forms[0].submit();
  	}
  	//删除
  	function doDelete(id){
  		document.forms[0].action = "${basePath}manage/info_delete.action?info.infoId=" + id;
  		document.forms[0].submit();
  	}
  	//批量删除
  	function doDeleteAll(){
  		document.forms[0].action = "${basePath}manage/info_deleteSelected.action";
  		document.forms[0].submit();
  	}
  	...
  	function doPublic(infoId, state){
  		//1、更新信息状态
  		$.ajax({
  			url:"${basePath}manage/info_publicInfo.action",
  	...
  	var list_url="${basePath}manage/info_listUI.action";
  	function doSearch(){
  		$("#pageNo").val(1);
